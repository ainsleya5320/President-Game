...
(async()=>{
 ...
 try{
  ...
  const Sim=vm.runInNewContext(fs.readFileSync(path.join(__dirname,'simulation-core.js'),'utf8')+'\nFourYearsSim');
  const old=Sim.fresh();old.version=2;old.quarter=6;for(const key of ['relationships','appointments','promises','goodwill','campaign'])delete old[key];
  ...
  assert.equal(saved.version,3);assert.equal(saved.quarter,6);assert.equal(saved.promises.length,1);assert.equal(saved.campaign.trips.length,1);assert.equal(saved.location,'aircraft');
  ...
  console.log('Full 3D page passed: v2 migration, room interactions, meeting, campaign travel, trip, return to Oval Office and save/resume.');
